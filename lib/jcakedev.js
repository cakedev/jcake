...
(function() {
  ...
  jcakedev.plugins.panel = {
    ...
    create: function($obj, params) {
      var closable, draggable, height, me, modal, position, title, width;
      me = this;
      title = params.title;
      ...
      width = params.width != null ? "" + params.width + "px" : null;
      height = params.height != null ? "" + params.height + "px" : null;
      position = params.position === "fixed" ? "fixed" : "absolute";
      return $obj.each(function() {
        var panel;
  ...
  Panel = (function() {
    ...
    function Panel(el, title, modal, draggable, closable, width, height, position) {
      ...
      this.panel.css("position", this.position);
      if (this.modal) {
        ...
        $wrapperContent.on("click", function(event) {
          if (me.closable && $(event.target).hasClass("-cakedev-panel-wrapper-content")) {
            return me.hide();
          }
        });
      } else {
      ...
      this.content = $("<div class='-cakedev-panel-content' />");
    ...
    Panel.prototype.onMousemove = function(event) {
      var containerHeight, containerWidth, left, top;
      if (this.dragging) {
        ...
        if (top < 0) {
          top = 0;
        } else if (top + this.panel.outerHeight() > containerHeight) {
          top = containerHeight - this.panel.outerHeight();
        }
        if (left < 0) {
          left = 0;
        } else if (left + this.panel.outerWidth() > containerWidth) {
          left = containerWidth - this.panel.outerWidth();
        }
  ...
  $(document).on("mouseup", function() {
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      cmp = _ref[_i];
      if (cmp instanceof Panel) {
        _results.push(cmp.setDragging(false));
      } else {
        _results.push(void 0);
      }
    }
